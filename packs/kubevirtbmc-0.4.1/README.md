...
This pack packages chart version `0.4.1`, which deploys KubeVirtBMC `v0.7.1`.
...
| `charts.kubevirtbmc.image.tag` | Controller image tag. Empty means the chart's `appVersion` (`v0.7.1`). | String | `""` | No |
...
- Helm does not upgrade CRDs that ship in a chart's `crds/` directory. `VirtualMachineBMC` gained new
  ...
  ```shell
  kubectl apply -f https://raw.githubusercontent.com/kubevirtbmc/kubevirtbmc/v0.7.1/config/crd/bases/bmc.kubevirt.io_virtualmachinebmcs.yaml
  ```
