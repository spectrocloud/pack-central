{{- define "appdynamics-cloud-k8s-monitoring.sensitiveData" -}}
{{ (get . "data") | trim | b64enc | required (get . "message") }}
{{- end -}}

{{- define "appdynamics-cloud-k8s-monitoring.podConfigs" }}
  image: {{ .image }}
...
{{- define "appdynamics-cloud-k8s-monitoring.osNodeSelector" }}
nodeSelector:
  kubernetes.io/os: {{ . }}
{{- end }}
...
{{- define "appdynamics-cloud-k8s-monitoring.getClusterName" }}
{{- if .Values.global.smartAgentInstall -}}
{{ "AGENT_PLATFORM_NAME_VALUE" }}
{{- else }}
{{- .Values.global.clusterName | required "clusterName needs to be specified" }}
{{- end }}
{{- end }}

{{- define "appdynamics-cloud-k8s-monitoring.getNamespace" }}
{{- if .Values.global.smartAgentInstall -}}
...
{{- define "appdynamics-cloud-k8s-monitoring.getOAuth" }}
oauth:
  {{ if .Values.global.smartAgentInstall -}}
  # If installation is via the smartagent, use the special purpose value to set what is being used by the smartagent.
  clientId: {{ "OAUTH_ID_VALUE" }}
  clientSecret: {{ "OAUTH_SECRET_PLAIN_VALUE" }}
  endpoint: {{ "SERVICE_DOMAIN_VALUE" }}/data
  tokenUrl: {{ "OAUTH_URL_VALUE" }}
  {{ else }}
  {{ with .Values.global.oauth -}}
  # Any one of clientIdEnvVar or clientId is required
  {{ if .clientIdEnvVar -}}
  clientIdVarSource: {{- toYaml .clientIdEnvVar.valueFrom | nindent 4  }}
  {{ else }}
  clientId: {{ required "One of clientId or clientIdEnvVar is required when agent management is enabled" .clientId }}
  {{- end }}
  ...
  {{ else }}
  clientSecret: {{ required "One of clientSecret or clientSecretEnvVar is required when agent management is enabled" .clientSecret }}
  {{- end }}
...
{{- define "appdynamics-cloud-k8s-monitoring.getAgentManagementProxy" }}
agentManagementProxy:
  {{ if .Values.global.smartAgentInstall -}}
  # If installation is via the smartagent, use the special purpose value to set what is being used by the smartagent.
  httpProxy: {{ "AGENT_HTTP_PROXY_VALUE" }}
  httpsProxy: {{ "AGENT_HTTPS_PROXY_VALUE" }}
  {{ else }}
...
{{- define "appdynamics-cloud-k8s-monitoring.appdLogsCoditionalConfig"}}
conditionalConfigs:
  - condition:
      or:
        - contains:
...
{{- define "appdynamics-cloud-k8s-monitoring.logCollectorConditionProcessors" }}
{{- if .logFormat}}
- add_fields:
    target: appd
    fields:
...
{{- $allParsers := (list .logback .log4J .json .grok .infra .multi .timestamp) }}
{{- range $allParsers }}
{{- if . }}
{{- if .enabled }}
...
{{- define "appdynamics-cloud-k8s-monitoring.filebeatYml" }}
{{- $clusterName := "" }}
{{- if .Values.global.smartAgentInstall }}
# If installation is via the smartagent, use the special purpose value to set what is being used by the smartagent.
{{- $clusterName = "AGENT_PLATFORM_NAME_VALUE" }}
{{- else }}
{{- $clusterName = .Values.global.clusterName | required "clusterName needs to be specified" }}
{{- end }}
{{- $clusterId := "" }}
{{- if .Values.global.smartAgentInstall }}
# If installation is via the smartagent, use the special purpose value to set what is being used by the smartagent.
...
{{- $_:=set $containerConfig "conditionalConfigs" (concat $containerConfig.conditionalConfigs (include "appdynamics-cloud-k8s-monitoring.appdLogsCoditionalConfig" .| fromYaml).conditionalConfigs) }}
{{- end }}
  ...
  providers:
    - type: kubernetes
      ...
      hints.default_config:
        ...
        processors:
          {{- include "appdynamics-cloud-k8s-monitoring.logCollectorConditionProcessors" . | nindent 10 }}
      {{- end}}
      {{- else}}
        ...
        - condition:
          ...
          config:
            ...
            - type: filestream
              ...
              prospector.scanner.symlinks: true
              processors:
                {{- include "appdynamics-cloud-k8s-monitoring.logCollectorConditionProcessors" . | nindent 16 }}
            {{- end}}
            {{- end}}
        {{- end}}
