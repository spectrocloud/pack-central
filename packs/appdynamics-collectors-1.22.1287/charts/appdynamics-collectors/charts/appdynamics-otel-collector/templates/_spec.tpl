...
exporters:
  otlphttp:
    metrics_endpoint: {{ include "appdynamics-otel-collector.endpoint" . }}/v1/metrics
    traces_endpoint: {{ include "appdynamics-otel-collector.endpoint" . }}/v1/trace
    logs_endpoint: {{ include "appdynamics-otel-collector.endpoint" . }}/v1/logs
    retry_on_failure:
    ...
    metric_statements:
      ...
      - context: datapoint
        statements:
          ...
          {{- end }}
          - set(resource.attributes["k8s.cluster.name"], {{ include "appdynamics-otel-collector.cluster.name" . | quote }})
          - set(resource.attributes["k8s.cluster.id"], {{ include "appdynamics-otel-collector.clusterId" . | quote}})
service:
  pipelines:
    metrics/self:
...
extensions:
  appdagentmanagementextension:
    service_url: {{ include "appdynamics-otel-collector.serviceURL" . }}
    ...
    {{- (include "appdynamics-otel-collector.derivedOAuth" .) | nindent 6}}
      tenant_id: {{ include "appdynamics-otel-collector.tenant.id" . }}
    platform:
      id: {{ (include "appdynamics-otel-collector.clusterId" .) | quote }}
      name: {{ (include "appdynamics-otel-collector.cluster.name" .) | quote }}
      type: k8s
    deployment:
      scope: {{ include "appdynamics-otel-collector.namespace" .}}
      unit: "${POD_NAME}"
      unique: false
    helm_chart_version: {{ .Values.global.helmChartVersion }}
    http_client_settings:
    {{- (include "appdynamics-otel-collector.client.agent.proxy" .) | nindent 6}}
service:
...
exporters:
{{- .Values.presets.selfTelemetry.exporters | toYaml | nindent 2}}
...
{{- $exporter_list := list }}
{{- range $k,$v:= $.Values.presets.selfTelemetry.exporters }}
{{- $exporter_list = append $exporter_list $k }}
{{- end }}
      exporters:
{{- $exporter_list | toYaml | nindent 8}}
...
{{- define "appdynamics-otel-collector.gomemlimit" -}}
name: GOMEMLIMIT
value: "{{ include "appdynamics-otel-collector.getMemLimitMib" .spec.resources.limits.memory }}MiB"
{{- end }}
...
{{/*
  The env var that may be different from gateway collector and sampler collector
  variable
    spec - the collector spec
    Values - global Values
*/}}
{{- define "appdynamics-otel-collector.appendGoMemLimitEnv" -}}
{{- if .Values.useGOMEMLIMIT }}
{{- $specEnv := get .spec "env" | deepCopy }}
{{- $specEnv = append $specEnv  (include "appdynamics-otel-collector.gomemlimit" . | fromYaml)}}
env:
{{- $specEnv | toYaml | nindent 2}}
{{- end }}
{{- end }}
