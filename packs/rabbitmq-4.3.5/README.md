...
prepares the cluster to run RabbitMQ `4.3.5`.
...
instance — see [Usage](#usage) for how to create one pinned to `4.3.5`.
...
This version (`4.3.5`) succeeds `4.3.4`. The only change is the pinned RabbitMQ
image tag referenced in this README and in `values.yaml`; the Cluster Operator and
Messaging Topology Operator versions are unchanged.
...
The image tag above (`4.3.5-management-alpine`) is also declared in
