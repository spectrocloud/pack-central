...
- This release (`fluentbit-5.1.1`) updates the Fluent Bit image and underlying Helm chart from `0.57.9` (Fluent Bit `5.0.9`) to `0.58.1` (Fluent Bit `5.1.1`). Per the upstream chart [CHANGELOG](https://github.com/fluent/helm-charts/blob/main/charts/fluent-bit/CHANGELOG.md), the `0.58.0` and `0.58.1` releases only bump the Fluent Bit OCI image tag (to `v5.1.0` and `v5.1.1` respectively) — there are **no structural changes** to `values.yaml` between this pack and the previous `fluentbit-5.0.9` pack.
- The previous pack version, **`fluentbit-5.0.9`**, is available in this repository under [`packs/fluentbit-5.0.9`](../fluentbit-5.0.9) and can be reviewed as a compatibility/historical reference.
- Ensure compatibility with the Kubernetes version (1.29 or higher) before upgrading.
