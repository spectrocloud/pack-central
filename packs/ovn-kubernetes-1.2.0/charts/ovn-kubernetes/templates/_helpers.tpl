{{/*
Discover Kubernetes API server URL from the default/kubernetes Endpoints, or use
.Values.k8sAPIServer if set (override for helm template / when lookup is not available).
*/}}
{{- define "getK8sAPIServer" -}}
{{- if .Values.k8sAPIServer -}}
{{- .Values.k8sAPIServer -}}
{{- else -}}
{{- $ep := lookup "v1" "Endpoints" "default" "kubernetes" -}}
{{- if and $ep $ep.subsets (index $ep.subsets 0) (index (index $ep.subsets 0).addresses 0) -}}
{{- $addr := index (index $ep.subsets 0).addresses 0 -}}
https://{{ $addr.ip }}:6443
{{- end -}}
{{- end -}}
{{- end }}
