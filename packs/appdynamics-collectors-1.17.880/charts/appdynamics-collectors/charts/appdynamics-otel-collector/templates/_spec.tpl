...
processors:
  resource/ebpf:
    attributes:
      ...
      - key: net.platform.name
        action: upsert
        value: {{ .Values.global.clusterName | quote }}
      - key: net.platform.type
        action: upsert
        value: k8s
  filter/ebpf:
    metrics:
  ...
  transform/ebpf:
    metric_statements:
      - context: datapoint
        statements:
          - convert_sum_to_gauge() where IsMatch(metric.name, "tcp.active|tcp.rtt.num_measurements|tcp.rtt.average|udp.active|http.active_sockets|dns.active_sockets")
          - set(start_time_unix_nano, time_unix_nano-Nanoseconds(Duration("30s"))) where metric.type == 2
  attributes/ebpf:
    actions:
...
service:
  pipelines:
    metrics/ebpf:
      receivers: [otlp/ebpf]
      processors: [memory_limiter, resource/ebpf, filter/ebpf, transform/ebpf, attributes/ebpf, batch/metrics]
      exporters: [otlphttp]
{{- end }}
...
processors:
  ...
  {{- end }}
  {{- if .Values.presets.truncated.metric.metric }}
    - context: metric
      statements:
        - truncate_all(attributes, {{.Values.presets.truncated.metric.metric}})
  {{- end }}
